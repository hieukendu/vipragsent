...
As of `2026-08-16T15:07:31Z`, the strongest defensible result is `PROJECTED_GATE_CONDITIONAL`, not a measured campaign duration. The live run has no active PID, the production worktree is dirty, loaded-code identity is uncertain, telemetry is partial, and the code-only phase forbids real production-model/data benchmarks and real Azure calls.
