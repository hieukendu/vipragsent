# ViPragSent sequential experiment run: q4_vipragsent_full_vistral_20260522

This runbook names exactly one inventory entry. Do not start another experiment or Azure job.

## Locked entry

- Experiment ID: `q4_vipragsent_full_vistral_20260522`
...
- Reusable checkpoint key: `vipragsent_full_vistral:20260522`
...
1. Run `python scripts/run_single_experiment.py --experiment-id q4_vipragsent_full_vistral_20260522 --stage preflight`.
2. If preflight is BLOCKED or FAIL, paste its report into the Codex chat and stop.
3. After preflight passes, run `python scripts/run_single_experiment.py --experiment-id q4_vipragsent_full_vistral_20260522 --stage all`.
4. On interruption, resume only this entry with `python scripts/run_single_experiment.py --experiment-id q4_vipragsent_full_vistral_20260522 --resume`.
...
Print the complete review summary with `python scripts/print_run_review_summary.py --run-id q4_vipragsent_full_vistral_20260522` and paste it into the Codex chat. It must include `RUN_STATUS`, `USER_REVIEW_STATUS`, `NEXT_RUN_ALLOWED`, artifact hashes, and blockers.
