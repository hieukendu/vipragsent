# ViPragSent sequential Azure job: azure_gpt41_mini_8shot

This runbook names exactly one Azure job. Do not start another job, experiment, batch, or global matrix.

## Locked job

- Job ID: `azure_gpt41_mini_8shot`
- Job type: `pragmatic_8_shot`
...
1. Run `python scripts/run_single_azure_job.py --job-id azure_gpt41_mini_8shot --stage preflight`.
...
4. Run that locked sequence with `python scripts/run_single_azure_job.py --job-id azure_gpt41_mini_8shot --stage all`.
5. On interruption, resume only this job with `python scripts/run_single_azure_job.py --job-id azure_gpt41_mini_8shot --resume`.
...
Complete the sequential stages applicable to this job, print the complete review summary with `python scripts/print_run_review_summary.py --run-id azure_gpt41_mini_8shot`, and paste it into the Codex chat. Include request/token usage, invalid-output accounting when applicable, artifact hashes, `RUN_STATUS`, `USER_REVIEW_STATUS`, and `NEXT_RUN_ALLOWED`.
