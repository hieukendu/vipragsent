...
1. Run `python scripts/run_single_azure_job.py --job-id azure_q3_pragmatic_8_shot_256 --stage preflight`.
...
4. Run that locked sequence with `python scripts/run_single_azure_job.py --job-id azure_q3_pragmatic_8_shot_256 --stage all`.
5. On interruption, resume only this job with `python scripts/run_single_azure_job.py --job-id azure_q3_pragmatic_8_shot_256 --resume`.
...
Complete the sequential stages applicable to this job, print the complete review summary with `python scripts/print_run_review_summary.py --run-id azure_q3_pragmatic_8_shot_256`, and paste it into the Codex chat. Include request/token usage, invalid-output accounting when applicable, artifact hashes, `RUN_STATUS`, `USER_REVIEW_STATUS`, and `NEXT_RUN_ALLOWED`.
