...
1. Run `python scripts/run_single_azure_job.py --job-id azure_pragmatic_8_shot --stage preflight`.
2. If preflight is BLOCKED or FAIL, paste the report into the Codex chat and stop.
3. After preflight passes, run `python scripts/run_single_azure_job.py --job-id azure_pragmatic_8_shot --stage all`.
4. On interruption, resume only this job with `python scripts/run_single_azure_job.py --job-id azure_pragmatic_8_shot --resume`.
