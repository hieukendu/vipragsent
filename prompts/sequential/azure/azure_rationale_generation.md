# ViPragSent sequential Azure job: azure_rationale_generation

This runbook names exactly one Azure job. Do not start another job, experiment, batch, or global matrix.

## Locked job

- Job ID: `azure_rationale_generation`
- Job type: `rationale_generation`
- Research question: `setup`
- Task: `rationale`
- Budget: ``
- Split: `vipragsent_train`
- Required assets: `azure_deployment;prompt_manifest`
- Model family: `gpt-4.1-mini`

## Required command sequence

1. Run `python scripts/run_single_azure_job.py --job-id azure_rationale_generation --stage preflight`.
...
4. Run that locked sequence with `python scripts/run_single_azure_job.py --job-id azure_rationale_generation --stage all`.
5. On interruption, resume only this job with `python scripts/run_single_azure_job.py --job-id azure_rationale_generation --resume`.
...
Complete the sequential stages applicable to this job, print the complete review summary with `python scripts/print_run_review_summary.py --run-id azure_rationale_generation`, and paste it into the Codex chat. Include request/token usage, invalid-output accounting when applicable, artifact hashes, `RUN_STATUS`, `USER_REVIEW_STATUS`, and `NEXT_RUN_ALLOWED`.
