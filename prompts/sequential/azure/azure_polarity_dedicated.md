# ViPragSent sequential Azure job: azure_polarity_dedicated

This runbook names exactly one Azure job. Do not start another job, experiment, batch, or global matrix.

## Locked job

- Job ID: `azure_polarity_dedicated`
- Job type: `polarity_dedicated_prompt`
...
1. Run `python scripts/run_single_azure_job.py --job-id azure_polarity_dedicated --stage preflight`.
...
4. Run that locked sequence with `python scripts/run_single_azure_job.py --job-id azure_polarity_dedicated --stage all`.
5. On interruption, resume only this job with `python scripts/run_single_azure_job.py --job-id azure_polarity_dedicated --resume`.
