...
REQUIRED_COLUMNS = {
    ...
    "q4_calibration.csv": "system,backbone,polarity_test_ece,bin_count,binning,confidence_definition,temperature_scaling,seed_count",
...
def validate_artifact_tree(root: str | Path) -> list[str]:
    ...
    for filename, columns in REQUIRED_COLUMNS.items():
        directory = "backing_data" if filename in {"q3_low_resource.csv", "split_and_label_counts.csv"} else "tables"
